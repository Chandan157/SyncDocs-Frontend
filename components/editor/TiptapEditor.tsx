...
export default function TiptapEditor({ documentId }: { documentId: string }) {
  ...
  const editor = useEditor({
    ...
    onTransaction: ({ transaction, editor }) => {
      // If this transaction was generated by OUR local user typing
  ...
  useEffect(() => {
    ...
    db.documents.get(documentId).then(localDoc => {
      ...
      setIsReady(true);
      editor.setEditable(true); // Allow immediate offline editing
    }).catch(console.error);
  ...
  useEffect(() => {
    ...
    client.onDocumentLoaded = (serverContent: string, role?: string) => {
      ...
      if (!serverEmpty && !localEmpty && rawServer !== rawLocal && role !== 'viewer') {
        // We have local edits that differ from the server's canonical state
    ...
    client.onIncomingOperation = (op: Operation) => {
      ...
      // Symmetrical application of incoming OT operations
      if (op.type === 'replace' && op.position === 0) {
        isApplyingRef.current = true;
        
        // Save precise cursor selection
        const { from, to } = editor.state.selection;
        
        // Apply incoming HTML and restore cursor synchronously
        editor.chain()
          .setContent(op.text || '')
  ...
  useEffect(() => {
    if (otClientRef.current) {
      ...
      // We also need to extract role from onDocumentLoaded, which was already updated earlier
      ...
      otClientRef.current.onReconnectDocumentLoaded = (serverContent, role) => {
        ...
        // Strip out tiptap's default empty paragraph artifacts for the exact match check
        const rawServer = (serverContent || '').replace(/<p><br><\/p>/g, '').replace(/<p><\/p>/g, '').trim();
        const rawLocal = (localContent || '').replace(/<p><br><\/p>/g, '').replace(/<p><\/p>/g, '').trim();

        // Show conflict ONLY if both have real content, and their HTML content actually differs
        if (!serverEmpty && !localEmpty && rawServer !== rawLocal && role !== 'viewer') {
          setConflictState({ serverContent, localContent });
        } else {
          if (serverEmpty && !localEmpty) {
